        ...
        <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
            ...
            {cartData.length == 0 ? (
                ...
                >
                    ...
                    <Link href={"/"} className='bg-green-600 text-shadow-white px-6 py-3 rounded-full hover:bg-green-700 transition-all inline-block font-medium'>Continue Shopping</Link>
                        ...
                        <div className='lg:col-span-2 space-y-5'>
                            <AnimatePresence>
                                {cartData.map((item, index) => (
                                    ...
                                    >
                                        <div className='relative w-28 h-28 sm:w-24 sm:h-24 md:w-28 md:h-28 flex-shirink-0 rounded-xl overflow-hidden bg-gray-50'>
                                        ...
                                        <div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full'>
                                            <button className='bg-white  p-1.5 rounded-full hover:bg-green-100 transition-all border border-gray-200' onClick={()=>dispatch(decreaseQuantity(item._id))}><Minus className='text-green-700' size={14}/></button>
                                            <span className='font-semibold w-6 text-center text-gray-800 '>{item.quantity}</span>
                                            <button className='bg-white  p-1.5 rounded-full hover:bg-green-100 transition-all border border-gray-200' onClick={()=>dispatch(increaseQuantity(item._id))}><Plus className='text-green-700' size={14}/></button>
                                        </div>
                                        <button className='sm:ml-4 mt-3 sm:mt-0 text-red-500 hover:text-red-700 transition-all'onClick={()=>dispatch(removeFromCart(item._id))}><Trash2/></button>
                                    </motion.div>
                                ))}
                            </AnimatePresence>
                        </div>
